...
- The code evolves as I work through puzzles. You may see helper functions and experimental code left in files; these are intentional while iterating on solutions.
- If you want me to standardise the day folders (flags for input, tests, README per day), tell me which days and I can apply consistent scaffolding.

If you'd like, I can:

- Add a small `Makefile` or top-level script to run any day by number (e.g. `./run 1` runs `day1`).
- Add a `README.md` inside each `dayN` with a short description and sample run command.
- Convert input toggling to a `-input` flag and add tests for selected days.
